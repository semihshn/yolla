...
The selected consumer approach is to make the listener visible as a real use case: `OrderingDomainEventListenerAdapter` maps the Kafka contract to a payment model and delegates to `PaymentService.pay`. The payment service persists a completed payment and is idempotent by `orderId`, which is required because the externalized publication is at-least-once.
...
OrderController -> OrderService.createOrder() @Transactional
                         ...
                         publication is marked completed
                                      |
                                      v
                         payment-api listener -> PaymentService.pay()
                         -> payments row
```
...
- The payment API has a Kafka listener, payment service, JPA adapter and `payments` table so the consumer side is a real use case rather than a logging-only example.
- Kafka is selected by `spring-modulith-events-kafka`; the broker address is selected by `spring.kafka.bootstrap-servers`. RabbitMQ alternatives remain commented in the producer and consumer configuration.

## Dependency and compatibility decisions

- Keep Java 17 and Gradle 8.4. Align the Spring Boot line to `3.2.12` because Spring Modulith `1.1.12` is compiled against the Spring Boot 3.2 line.
- Align Spring Cloud to `2023.0.5` / `4.1.5` so the existing bootstrap dependency remains compatible with Spring Boot 3.2.12.
- Use Spring Modulith `1.1.12`, which provides `@Externalized`, JPA event publications and Kafka externalization.
...
4. A lightweight application-context/integration test will verify the Spring Modulith configuration when the local test dependencies are available. The test will avoid requiring a live Kafka broker and will focus on event publication wiring.
5. Payment domain and listener tests will verify completion, idempotency, event-to-payment mapping and acknowledge-after-success behavior; the manual README flow will cover broker delivery.

The initial test attempt exposed a dependency alignment issue: Modulith 1.1.12 was combined with the repository's old Boot 3.1 line. The branch now pins compatible Boot and Cloud versions, and the final verification records the resulting test commands and outputs.
...
- A real payment consumer use case with a Kafka listener, idempotent payment service and payment persistence.
- Database/Kafka local setup and concise branch README documentation.
- Focused unit and web tests.

Not included:

- Inventory or shipping consumers.
...
- `outbox-with-annotation` is based on `main` and contains a complete Order API plus payment consumer example.
...
- The payment listener delegates to `PaymentService.pay`, persists a completed payment and does not duplicate it on event redelivery.
